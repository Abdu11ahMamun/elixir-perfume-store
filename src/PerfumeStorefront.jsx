    ...
    <div className="bg-[#F7F3EF] text-[#1F1F1F] min-h-screen font-sans">
      <header className="sticky top-0 z-50 bg-[#F7F3EF]/90 backdrop-blur border-b border-[#e9dfd7]">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-10 text-sm tracking-wide uppercase">
            <a href="#" className="hover:text-[#b27b52] transition">Home</a>
            <a href="#" className="hover:text-[#b27b52] transition">Collections</a>
            <a href="#" className="hover:text-[#b27b52] transition">Best Sellers</a>
            <a href="#" className="hover:text-[#b27b52] transition">About</a>
          </nav>
          ...
          <div className="flex items-center px-8 lg:px-20 py-16 bg-[#efe7df]">
            <div className="max-w-xl">
              ...
              <div className="flex gap-4">
                <button className="bg-[#1F1F1F] text-white px-7 py-4 rounded-full hover:bg-[#b27b52] transition">
                  Explore Collection
                </button>
                <button className="border border-[#1F1F1F] px-7 py-4 rounded-full hover:bg-white transition">
                  View Best Sellers
                </button>
              </div>
            </div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-24">
        <div className="flex items-center justify-between mb-12">
          ...
          <button className="hidden md:block border border-[#1F1F1F] px-5 py-2 rounded-full hover:bg-[#1F1F1F] hover:text-white transition">
            View All
          </button>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {collections.map((item) => (
            <div key={item.title} className="relative rounded-[2rem] overflow-hidden group h-[450px]">
      ...
      <section className="bg-white py-24">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <p className="uppercase text-sm tracking-[0.3em] text-[#b27b52] mb-3">Best Sellers</p>
            <h3 className="text-5xl font-light mb-4">Featured Fragrances</h3>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {featured.map((item) => (
              <div key={item.name} className="group bg-[#faf7f3] rounded-[2rem] overflow-hidden hover:-translate-y-2 transition duration-500 shadow-sm hover:shadow-xl">
                <div className="overflow-hidden">
                  <img src={item.image} alt={item.name} className="h-[340px] w-full object-cover group-hover:scale-105 transition duration-700" />
                </div>
                <div className="p-6">
                  <p className="text-xs uppercase tracking-[0.25em] text-[#b27b52] mb-2">Eau De Parfum</p>
                  <h4 className="text-2xl font-light mb-2">{item.name}</h4>
                  <p className="text-gray-500 mb-5">{item.note}</p>
                  <div className="flex items-center justify-between">
                    <span className="text-xl font-semibold">{item.price}</span>
                    <button onClick={() => addToCart(item)} className="bg-[#1F1F1F] text-white px-4 py-2 rounded-full hover:bg-[#b27b52] transition">
                      Add to Cart
                    </button>
                  </div>
                </div>
      ...
      <section className="max-w-7xl mx-auto px-6 py-24 grid lg:grid-cols-2 gap-14 items-center">
        <div className="relative rounded-[2rem] overflow-hidden h-[600px]">
          <img src="https://images.unsplash.com/photo-1616949755610-8c9bbc08f138?q=80&w=1400&auto=format&fit=crop" alt="Luxury perfume" className="absolute inset-0 w-full h-full object-cover" />
        </div>
        <div>
          <p className="uppercase text-sm tracking-[0.3em] text-[#b27b52] mb-4">Our Philosophy</p>
          ...
          <p className="text-gray-600 leading-8 mb-6 text-lg">
            This design combines the elegance of luxury perfume brands with the simplicity of modern ecommerce.
          </p>
        ...
        <section className="fixed inset-0 z-[100] bg-black/50 flex justify-end">
          <div className="bg-[#F7F3EF] w-full max-w-xl h-full overflow-y-auto shadow-2xl p-6 md:p-8">
            <div className="flex items-center justify-between mb-8">
              <div>
                <p className="uppercase tracking-[0.3em] text-xs text-[#b27b52] mb-2">Checkout</p>
                <h3 className="text-4xl font-light">Your Cart</h3>
              </div>
              <button onClick={() => setIsCartOpen(false)} className="w-10 h-10 rounded-full border border-[#1F1F1F] hover:bg-[#1F1F1F] hover:text-white transition">
                ×
              </button>
            </div>
            ...
            {cart.length === 0 && !orderPlaced ? (
              <div className="bg-white rounded-[2rem] p-8 text-center shadow-sm">
                ...
                <button onClick={() => setIsCartOpen(false)} className="bg-[#1F1F1F] text-white px-6 py-3 rounded-full hover:bg-[#b27b52] transition">
                  Continue Shopping
                </button>
              </div>
            ) : cart.length > 0 ? (
              <>
                <div className="space-y-4 mb-8">
                  {cart.map((item) => (
                    <div key={item.name} className="bg-white rounded-3xl p-4 flex gap-4 shadow-sm">
                      ...
                      <div className="flex-1">
                        <div className="flex items-start justify-between gap-4">
                          <div>
                            ...
                            <p className="text-[#b27b52] font-semibold mt-2">{item.price}</p>
                          </div>
                          <button onClick={() => removeFromCart(item.name)} className="text-gray-400 hover:text-red-500 transition">
                            Remove
                          </button>
                        </div>
                        <div className="flex items-center gap-3 mt-4">
                          <button onClick={() => updateQuantity(item.name, "decrease")} className="w-8 h-8 rounded-full border border-gray-300 hover:bg-[#1F1F1F] hover:text-white transition">−</button>
                          <span>{item.quantity}</span>
                          <button onClick={() => updateQuantity(item.name, "increase")} className="w-8 h-8 rounded-full border border-gray-300 hover:bg-[#1F1F1F] hover:text-white transition">+</button>
                        </div>
                      </div>
                    </div>
                  ))}
      ...
      <section className="bg-[#1F1F1F] text-white py-24 px-6">
        <div className="max-w-4xl mx-auto text-center">
          <p className="uppercase tracking-[0.3em] text-sm text-[#d6b295] mb-4">Stay Updated</p>
          <h3 className="text-5xl font-light mb-6">Join The Fragrance Club</h3>
          <p className="text-gray-300 leading-8 mb-10">
            Get exclusive launches, special discounts, and curated perfume recommendations directly in your inbox.
          </p>
      ...
      <footer className="bg-[#111111] text-gray-400 py-12 px-6">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between gap-8">
          <div>
            <h4 className="text-2xl tracking-[0.25em] text-white font-light mb-4">ÉLIXIR</h4>
