...
export default function AdminCopyButton({ value, label = "Copy", copiedLabel = "Copied", className = "" }) {
  ...
  return (
    <button
      ...
      onClick={() => copy(value)}
      disabled={!value}
      className={`inline-flex shrink-0 items-center gap-1 rounded-md border border-gray-200 bg-white px-2 py-1 text-[11px] font-medium text-gray-500 transition hover:border-gray-300 hover:text-gray-700 disabled:cursor-not-allowed disabled:opacity-40 ${className}`}
    >
      {isCopied ? (
        <>
        ...
        <>
          <svg width="11" height="11" viewBox="0 0 12 12" fill="none">
            ...
            <path d="M2.5 8.5V2.5a1 1 0 011-1H8.5" stroke="currentColor" strokeWidth="1.2" />
          </svg>
          {label}
        </>
      )}
